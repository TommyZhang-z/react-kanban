...
const Column = ({ title, headingColor, column, cards, setCards }) => {
  ...
  const handleDrop = (e) => {
    ...
    if (before !== cardId) {
      let copy = [...cards];
      let card = copy.find((card) => card.id === cardId);
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="cursor-grabbing rounded border border-neutral-700 bg-neutral-800 p-3 active:cursor-grabbing"
      >
...
// FIXME： fix icon on drag over animation
const BurnBarrel = ({ setCards }) => {
  ...
  return (
    ...
    >
      {active ? <FaFire className=" animate-ping" /> : <FiTrash className="" />}
    </div>
  );
};
